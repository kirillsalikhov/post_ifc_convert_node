...
const runFullXmlGlbPipeline = (xmlFilePath, glbFilePath, outputDirPath) => {
    const tmpGltfFilePath = path.join(outputDirPath, 'model.glb.gltf');
    ...
    console.log(cp.spawnSync('npm', ['run', 'glb-gltf', '--', glbFilePath, tmpGltfFilePath], { shell: true, encoding: 'utf-8' }));
    console.log(cp.spawnSync(CONVERTER_CLI_PATH, getRenameGltfNodesArgs(tmpGltfFilePath, objectsPath, gltfFilePath), {encoding: 'utf-8'}));
};
